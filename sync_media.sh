...
timeout 300 rsync -az --ignore-existing --include='*/' --include='*.mp4' --exclude='*' $REMOTE:~/Code/safeduo_media/ "$LOCAL/" 2>/dev/null && echo "$(ts) media rsync ok" >> "$LOG"
timeout 300 rsync -az --ignore-existing --files-from=<(ssh -o BatchMode=yes -o ConnectTimeout=20 $REMOTE 'cd ~/safeduo/artifacts/viz/a8_v4demo && find . -maxdepth 1 -name "*.mp4" -mtime -3 -printf "%f\n"' 2>/dev/null) $REMOTE:~/safeduo/artifacts/viz/a8_v4demo/ "$LOCAL/a8_v4demo/" 2>/dev/null
cd "$MEDIA" || exit 1
